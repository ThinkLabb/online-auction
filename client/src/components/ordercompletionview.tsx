...
export function OrderCompletionView({
  ...
  const steps = [
    {
      ...
      completed:
        !!order.shipping_address && order.status !== "pending_payment",
    },
    {
      number: 2,
  ...
  return (
    <div className="container mx-auto py-8 px-4 flex flex-col gap-4 max-w-4xl">
                ...
                {step.number === 1 && isBuyer && order.status === "pending_payment" && (
                  <div className={`${MUTE} p-4 rounded-lg space-y-3`}>
                    ...
                    <div
                      ...
                      onDrop={(e) => { e.preventDefault(); e.currentTarget.classList.remove('bg-blue-50', 'border-blue-400'); handlePaymentInvoiceChange({ target: { files: e.dataTransfer.files } } as any); }}
                    ...
                    >
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="text-gray-500 text-sm">Drag or click to upload payment invoice</p>
                      </div>
                    </div>
                    )}
                    <input
                    ...
                    {paymentInvoicePreview && (
                      <div className="relative">
                        <img src={paymentInvoicePreview} className="mt-2 h-32 object-contain border rounded" />
                        <button
                ...
                {step.number === 2 && order.is_seller && order.status === "payment_confirmed" && (
                  <div className="space-y-3">
                    ...
                    <div
                      onDragOver={(e) => { e.preventDefault(); e.currentTarget.classList.add('bg-blue-50', 'border-blue-400'); }}
                      onDragLeave={(e) => { e.currentTarget.classList.remove('bg-blue-50', 'border-blue-400'); }}
                      onDrop={(e) => { e.preventDefault(); e.currentTarget.classList.remove('bg-blue-50', 'border-blue-400'); handleShippingInvoiceChange({ target: { files: e.dataTransfer.files } } as any); }}
                    ...
                    >
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <p className="text-gray-500 text-sm">Drag or click to upload shipping invoice</p>
                        </div>
                    </div>
                    )}
                    <input
                    ...
                    {shippingInvoicePreview && (
                      <div className="relative">
                        <img src={shippingInvoicePreview} className="mt-2 h-32 object-contain border rounded" />
                        <button
      ...
      {showChat && (
        <OrderChat orderId={order.order_id} cur_id={order.cur_user_id} partner_name={order.partner_name} partner_id={order.partner_id} cur_name={order.cur_name} />
      )}
    </div>
  )
}

         ...
         case "EnterEmail":
            return (
               <div className='p-8 border border-gray-200 shadow-lg rounded-lg bg-white flex flex-col gap-4'>
                  ...
                  <hr/>
                  <form onSubmit={(e) => {e.preventDefault(); onSubmitEmail()}} className="flex flex-col gap-2">
                     <label htmlFor="email" className="text-muted-foreground">Enter your email address to get a code to verify your account.</label>
                     <input type="email" id="email" placeholder="your@example.com" className='w-full px-3 py-2 border rounded-md focus:outline-2 focus:outline-[#8D0000]' onChange={(e) => {setEmail(e.target.value)}}/>
                     {error && <span className="text-[#8D0000]">{error}</span>}
                     <div className="flex flex-row gap-4 w-full justify-end mt-4">
                        <div onClick={() => navigate("/signin")} className={`w-fit bg-black font-bold text-white py-2 px-4 rounded-md transition-color hover:cursor-pointer`}>
                        Cancel
                        </div>
         ...
         case "EnterCode":
            return (
               <div className='p-8 border border-gray-200 shadow-lg rounded-lg bg-white flex flex-col gap-4'>
                  ...
                  <hr/>
                  <form onSubmit={(e) => {e.preventDefault(); onSubmitCode()}} className="flex flex-col gap-2">
                     <label htmlFor="code" className="text-muted-foreground">Please check your emails for a message with your code. Your code is 6 numbers long.</label>
                     <input type="number" id="code" placeholder="123456" className='w-full px-3 py-2 border rounded-md focus:outline-2 focus:outline-[#8D0000]' onChange={(e) => setCode(e.target.value)}/>
                     {error && <span className="text-[#8D0000]">{error}</span>}
                     
                     <div onClick={onSubmitEmail}  className="font-semibold text-blue-600 hover:text-blue-700 transition-colors self-start hover:cursor-pointer">
                        {loading ? <ClipLoader loading={loading} size={20} color='white'/> : <p>Resend code?</p> }
                     </div>

                     <div className="flex flex-row gap-4 w-full justify-end mt-4">
                        <div onClick={() => setCurPage("EnterEmail")} className={`w-fit bg-black font-bold text-white py-2 px-4 rounded-md transition-color hover:cursor-pointer`}>
                           Back
                        </div>
                        <button type="submit" className={`w-fit bg-[#8D0000] font-bold text-white py-2 px-4 rounded-md transition-color hover:cursor-pointer`}>
                           {loading ? <ClipLoader loading={loading} size={20} color='white'/> : <p>Continue</p> }
